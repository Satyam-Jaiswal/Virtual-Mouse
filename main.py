...
while True:
    ...
    # Getting hand landmarks and setting boundary box for hand.
    landmarklist, boundarybox = detector.findPosition(img)
    ...
    if len(landmarklist) != 0:
        ...
        cv2.rectangle(img, (frameReduction, frameReduction), (width_of_the_cam -
                                                              frameReduction, height_of_the_cam - frameReduction), (255, 0, 255), 2)
        ...
        if fingers[1] == 1 and fingers[2] == 0:
            ...
            x3 = np.interp(x1, (frameReduction, width_of_the_cam -
                                frameReduction), (0, width_of_the_Screen))
            y3 = np.interp(y1, (frameReduction, height_of_the_cam -
                                frameReduction), (0, height_of_the_Screen))

            # for reducing mouse shakiness
            current_location_X = previous_location_X + \
                (x3-previous_location_X)/smoothening
            current_location_Y = previous_location_Y + \
                (y3 - previous_location_Y)/smoothening

            # mouse move operation using autopy
            autopy.mouse.move(width_of_the_Screen -
                              current_location_X, current_location_Y)
            cv2.circle(img, (x1, y1), 15, (255, 0, 255), cv2.FILLED)
            # Updating mouse loactions
    ...
    cv2.putText(img, str(int(FramesPerSecond)), (20, 50),
                cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 255), 3)

    # Listening Virtual Mouse via camera
    cv2.imshow("Virtual Mouse - Satyam Jaiswal", img)
    cv2.waitKey(1)
